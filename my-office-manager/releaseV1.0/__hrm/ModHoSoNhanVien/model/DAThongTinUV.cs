...
namespace DAO
{
    public class DAResume
    {
        ...
        )
        {
            ...
            fb.AddInParameter(cmd, "@I_HO_TEN", DbType.String, HelpString.TrimAllBlankAndCapitalWord(I_HO_TEN));
        ...
        #region Report
        #region Báo cáo thông tin ứng viên
        public DataSet GetThongTinUV(long IdUv)
        {
            DataSet ds = new DataSet();
            DatabaseFB db = HelpDB.getDatabase();
            DbCommand cmd = db.GetStoredProcCommand("NEW_PROCEDURE");
            db.AddInParameter(cmd, "@RID", DbType.Int64, IdUv);
            db.LoadDataSet(cmd, ds, "NEW_PROCEDURE");
            return ds;
        }
        #endregion
