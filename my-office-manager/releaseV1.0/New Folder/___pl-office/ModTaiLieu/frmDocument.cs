    ...
    public partial class frmDocument : XtraFormPL
    {
        ...
        public frmDocument()
        {
            ...
            grdThongtinTL._init(GroupElementType.ONLY_INPUT,
                "DM_TAI_LIEU", "ID", "NAME", new string[] { "NAME" }, new string[] { "Tên tài liệu" },
        ...
        private void Grid_ValidateRow(object sender, ValidateRowEventArgs e)
        {
            ...
            GridColumn clTentailieu = clview.Columns["NAME"];
            ...
            if (e.Valid == true)
            {
                ...
                //                                    grdThongtinTL.Grid.GetRowCellValue(grdThongtinTL.Grid.FocusedRowHandle, grdThongtinTL.Grid.Columns["NAME"]).ToString(),
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            if (LoaiTaiLieu.TreeList_1.Nodes.Count == 0)
            {
                ...
                HelpMsgBox.ShowNotificationMessage("Bạn cần phải thêm loại tài liệu trước.");
            }
            else
            {
        ...
        private void btnDel_MouseDown(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left)
            {
                ...
                {
                    HelpMsgBox.ShowNotificationMessage("Không có tài liệu để xóa, xin vui lòng chọn lại.");
                ...
                {
                    HelpMsgBox.ShowNotificationMessage("Bạn chưa chọn tài liệu để xóa, xin vui lòng chọn lại.");
                    this.grdThongtinTL.btnDel.Click -= new System.EventHandler(btnDel_Click);
                }
                else
                {
        ...
        private void btnUpdate_MouseDown(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left)
            {
                if (grdThongtinTL.Grid.RowCount == 0)
                {
                    HelpMsgBox.ShowNotificationMessage("Không có tài liệu để cập nhật, xin vui lòng chọn lại.");
                    this.grdThongtinTL.btnUpdate.Click -= new System.EventHandler(btnUpdate_Click);
                }
                else if (grdThongtinTL.Grid.IsGroupRow(grdThongtinTL.Grid.FocusedRowHandle))
                {
                    HelpMsgBox.ShowNotificationMessage("Bạn chưa chọn tài liệu cập nhật, xin vui lòng chọn lại.");
                    this.grdThongtinTL.btnUpdate.Click -= new System.EventHandler(btnUpdate_Click);
                }
                else
                {
