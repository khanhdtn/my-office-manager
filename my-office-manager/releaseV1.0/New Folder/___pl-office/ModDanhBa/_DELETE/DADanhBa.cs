...
namespace DAO
{
    public class DADanhBa : DAPhieu<DODanhBa>
    {
        ...
        public static DADanhBa Instance =new DADanhBa();

        private DADanhBa() : base() { }
        ...
        public override DODanhBa LoadAll(long IDKey)
        {
            DODanhBa phieu = this.Load(IDKey);
        ...
        public override DODanhBa Load(long IDKey)
        {
            ...
            using (reader)
            {
                if (reader.Read())
                {
                    DODanhBa data = (DODanhBa)this.Builder.CreateFilledObject(typeof(DODanhBa), reader);
                    return data;
                }
            }
            return new DODanhBa();
        ...
        public override bool Update(DODanhBa data)
        {            
            DatabaseFB db = DABase.getDatabase();
            db.OpenConnection();
        ...
        public override bool Validate(DODanhBa element)
        {
            throw new NotImplementedException();
        }
            ...
            foreach (DataRow row in Field_Name)
            {
                ...
                }))
                {
                    Rules.SetValue(new FieldNameCheck("NAME",
                        ...
                        new string[] { ErrorMsgLib.errorRequired("Tên liên hệ") },
                        new object[] { }), i);
                    break;
                }
                if (lstRow.Exists(delegate(object e)
        ...
        public void CheckDuplicate(GridView view, DataSet GridDataSet, ValidateRowEventArgs e)
        {
            ...
            int count_Name,count_Tk,count_CMND ;
            count_Name = count_Tk = count_CMND = 0;
            foreach (DataRow r in GridDataSet.Tables[0].Rows)
            {
                if (r.RowState != DataRowState.Deleted)
                {
                    ...
                    if (r["NGUOI_DAI_DIEN"].ToString() == row["NGUOI_DAI_DIEN"].ToString() && r["NGUOI_DAI_DIEN"].ToString() != string.Empty)
                    {
                        if (view.IsNewItemRow(view.FocusedRowHandle))
                        {
                            row.SetColumnError("NGUOI_DAI_DIEN", "Thông tin \"Người liên lạc\" bị trùng, xin vui lòng kiểm tra lại!");
                            e.Valid = false;
                            return;
                        }
                        //else if (row.RowState == DataRowState.Modified || row.RowState == DataRowState.Added)
