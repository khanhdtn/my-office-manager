...
namespace office
{
    public partial class frmDiTreVeSom : XtraFormPL
    {
        ...
        {
            return MenuBuilder.CreateItem(typeof(frmDiTreVeSom).FullName,
                PMSupport.UpdateTitle("Đi trễ về sớm", Status),
                ParentID, true,
                typeof(frmDiTreVeSom).FullName,
        ...
        private void frmHotroSupport_Load(object sender, EventArgs e)
        {
            ...
            if (Phieu.NGAY_LAM_VIEC.ToString() != "01/01/0001 12:00:00 AM")
            {
                ...
                txtNguoiGhiNhan.Text = DAChiTietCongViec.Instance.GetNAME(Phieu.NGUOI_GHI_NHAN).Tables[0].Rows[0]["NAME"].ToString();
                PLMulDT_VS._setSelectedIDs(new long[] {Phieu.LOAI_DI_TRE_VE_SOM});
            }
            else
            {
                txtTG_GhiNhan.Text = DABase.getDatabase().GetSystemCurrentDateTime().ToString(PLConst.FORMAT_DATETIME_STRING);
                try
                {
                    Ngay_lien_quan.DateTime = System.Convert.ToDateTime(txtTG_GhiNhan.Text);
                }
                catch {
                    Ngay_lien_quan.DateTime = System.Convert.ToDateTime(txtTG_GhiNhan.Text + "M");
                }
                txtNguoiGhiNhan.Text = DAChiTietCongViec.Instance.GetNAME(FrameworkParams.currentUser.employee_id).Tables[0].Rows[0]["NAME"].ToString();                               
            }            
            ...
            CapNhat_TT.Text = Phieu.NGUOI_DUYET == 0 ? "" : DAChiTietCongViec.Instance.GetNAME(Phieu.NGUOI_DUYET).Tables[0].Rows[0]["NAME"].ToString();
            #endregion
        }                
        #endregion             
        ...
        private void btnSave_Click(object sender, EventArgs e)   
        {
            ...
            HelpInputData.TrimAllData(new object[] {this.rtxNoi_dung});
            GUIValidation.ShowRequiredError(Error, new object[] 
            { PLNguoiLQ, "Người liên quan",
              Ngay_lien_quan,"Ngày liên quan",              
              rtxNoi_dung,"Nội dung"  });
